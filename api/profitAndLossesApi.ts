...
import { PaginatedProfitAndLossList } from '../model/paginatedProfitAndLossList';
...
export enum ProfitAndLossesApiApiKeys {
    tokenAuth,
}

export class ProfitAndLossesApi {
    ...
    public setApiKey(key: ProfitAndLossesApiApiKeys, value: string) {
        (this.authentications as any)[ProfitAndLossesApiApiKeys[key]].apiKey = value;
    }
    ...
    /**
     * Returns a list of `ProfitAndLoss` objects.
     ...
     */
    public async profitAndLossesList (xAccountToken: string, createdAfter?: Date, createdBefore?: Date, cursor?: string, includeRemoteData?: boolean, modifiedAfter?: Date, modifiedBefore?: Date, pageSize?: number, remoteId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PaginatedProfitAndLossList;  }> {
        const localVarPath = this.basePath + '/profit-and-losses';
        ...
        if (xAccountToken === null || xAccountToken === undefined) {
            throw new Error('Required parameter xAccountToken was null or undefined when calling profitAndLossesList.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PaginatedProfitAndLossList;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "PaginatedProfitAndLossList");
    ...
    /**
     * Returns a `ProfitAndLoss` object with the given `id`.
     * @param xAccountToken Token identifying the end user.
     * @param id 
     * @param includeRemoteData Whether to include the original data Merge fetched from the third-party to produce these models.
     */
    public async profitAndLossesRetrieve (xAccountToken: string, id: string, includeRemoteData?: boolean, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: ProfitAndLoss;  }> {
        const localVarPath = this.basePath + '/profit-and-losses/{id}'
        ...
        // verify required parameter 'xAccountToken' is not null or undefined
        if (xAccountToken === null || xAccountToken === undefined) {
            throw new Error('Required parameter xAccountToken was null or undefined when calling profitAndLossesRetrieve.');
        }

        // verify required parameter 'id' is not null or undefined
        if (id === null || id === undefined) {
            throw new Error('Required parameter id was null or undefined when calling profitAndLossesRetrieve.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: ProfitAndLoss;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "ProfitAndLoss");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
